...
from myadmin.models import AdminUser, Subject, User
...
def index(request):
    ...
    all_user = users.all().count()  # 用户数量
    tests = subject.all().count()  # 试题数量
    request.session.set_expiry(0)  # 用户关闭浏览器自动关闭
    context = {"all_user": all_user, "tests": tests}
...
def dologin(request):
    try:
        ...
        # 根据登录账号获取登录者信息
        user = AdminUser.objects.get(username=request.POST['username'])
        # 判断当前用户是否是管理员
        if user.status == 6:
            # 判断登录密码是否相同
            import hashlib
...
def dissection(request):
    return render(request, 'myadmin/index/dissection.html')
